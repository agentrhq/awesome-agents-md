# AGENTS.md · Django 5.1 · DRF 3.15 · Celery 5.4 · Postgres 16 · pytest 8

## Stack

- **Python:** 3.12.x. **uv** for env and deps (`uv.lock` is source of truth). No `pip install` directly.
- **Framework:** Django 5.1, Django REST Framework 3.15.
- **Background jobs:** Celery 5.4 with Redis 7 broker. `flower` for inspection.
...
- Prod: `uv run gunicorn project.wsgi:application --workers 4`
...
- **Views:** prefer DRF `ViewSet` + `Router` over function views. Sync by default; DRF async support is partial.
...
- **Markers:** `@pytest.mark.slow` for tests >2s. Default run excludes them.
